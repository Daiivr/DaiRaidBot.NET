...
public static class RPEmbed
{
    public static Embed PokeEmbed(PKM pk, string username)
    {
        ...
        embed.AddField(x =>
        {
            ...
            x.Value = $"{Format.Bold($"Ability:")} {AbilityTranslationDictionary.AbilityTranslation[GameInfo.GetStrings(1).Ability[pk.Ability]]}\n{Format.Bold("Level:")} {pk.CurrentLevel}\n{Format.Bold("Nature:")} {NatureTranslations.TraduccionesNaturalezas[((Nature)pk.Nature).ToString()]}\n{Format.Bold("IVs:")} {pk.IV_HP}/{pk.IV_ATK}/{pk.IV_DEF}/{pk.IV_SPA}/{pk.IV_SPD}/{pk.IV_SPE}\n{MovesTranslationDictionary.MovesTranslation[GameInfo.GetStrings(1).Move[pk.Move1]]}";
            x.IsInline = true;
        });
